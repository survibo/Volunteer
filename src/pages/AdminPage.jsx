export default function AdminPage() {
  return (
    <section className="grid gap-6">
      <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-action-default">관리자</p>
      <h1 className="text-3xl font-bold leading-tight text-text-primary md:text-5xl">
        관리자 대시보드
      </h1>
      <div className="rounded-xl border border-border-default bg-surface-base p-6">
        <strong>관리 기능 준비 중</strong>
        <p className="mt-2 text-sm text-text-secondary">회원 승인, 봉사활동, 교육 관리는 이 영역에서 확장합니다.</p>
      </div>
    </section>
  )
}
